...
<aside class="relative" aria-label="Exam readiness preview">
	<div class="glass-card p-5 sm:p-6 border-lime-500/20">
		<div class="flex items-start justify-between gap-4 mb-5">
			<div>
				<p class="text-xs text-white/40 mb-1">Exam Readiness Score</p>
				<div class="font-display font-black text-4xl text-lime-400">
					{clampedScore}<span class="text-lg text-white/40">/100</span>
				</div>
			</div>
		...
		<div class="mb-5">
			<div class="mb-2 flex justify-between text-xs text-white/40">
				<span>Fail Zone</span>
				<span class="font-bold text-lime-400">75% Target</span>
				<span>100</span>
			</div>
			...
			{#each topics as topic}
				<div>
					<div class="mb-1.5 flex justify-between gap-3 text-xs">
						<span class="truncate text-white/65">{topic.label}</span>
						<span class="font-bold text-white/60">{topic.value}%</span>
					</div>
		...
		<div class="rounded-2xl border border-lime-500/20 bg-lime-500/10 p-4">
			<div class="flex items-start gap-3">
				<span class="text-xl" aria-hidden="true">🤖</span>
				<div>
					<div class="mb-1 text-xs font-black uppercase tracking-widest text-lime-400">AI Study Insight</div>
					<p class="text-xs leading-relaxed text-white/65">{insight}</p>
				</div>
			</div>
		</div>
	</div>

	<div class="absolute -right-3 -top-4 hidden rounded-2xl border border-lime-500/20 bg-black/70 px-4 py-2 shadow-xl backdrop-blur-xl sm:block">
		<div class="text-xs font-black text-white">🔥 {streak} Day Streak</div>
		<div class="text-[10px] text-white/40">Keep momentum</div>
	</div>
