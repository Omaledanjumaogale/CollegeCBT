...
async function getFirebase(): Promise<Auth | null> {
	...
	initPromise = (async () => {
		try {
			// Dynamic import of env vars from dynamic/public to avoid static analysis issues at edge
			const { env } = await import('$env/dynamic/public');
			const { PUBLIC_FIREBASE_API_KEY, PUBLIC_FIREBASE_AUTH_DOMAIN, PUBLIC_FIREBASE_PROJECT_ID, PUBLIC_FIREBASE_STORAGE_BUCKET, PUBLIC_FIREBASE_MESSAGING_SENDER_ID, PUBLIC_FIREBASE_APP_ID } = env as Record<string, string | undefined>;
