...
<svelte:head>
	<title>Super Admin Login — CollegeCBT | Enterprise Gate</title>
	...
	<div class="relative z-10 w-full max-w-[440px]" 
		...
		<div class="glass-card p-10 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
			...
			<div class="text-center mb-8">
				...
				<h1 class="font-display text-2xl mb-2 text-white">Super Admin Access</h1>
				<p class="text-white/40 text-sm">Enter secure credentials to bypass edge protocols.</p>
			</div>
			...
			>
				...
				<div>
					<label for="password" class="block text-[11px] font-bold text-white/40 uppercase tracking-widest mb-2 ml-1">Secure Password</label>
					<div class="relative group">
						<input 
							id="password"
							name="password" 
				...
				{#if form?.error}
					<div 
						class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-sm flex items-center gap-3 animate-in fade-in slide-in-from-top-2"
						role="alert"
					>
				...
				>
					...
					{:else}
						<span class="relative z-10">Bypass Security Gate →</span>
			...
			<div class="mt-8 pt-8 border-t border-white/5 flex items-center justify-between">
				...
				<p class="text-[10px] text-white/20 uppercase tracking-[0.2em] font-mono">CODE: ADMIN_V2.1</p>
			</div>
		</div>

		<!-- ── Footer Links ── -->
		<div class="mt-8 flex items-center justify-center gap-6">
			<a href="/" class="text-[11px] font-bold text-white/30 uppercase tracking-widest hover:text-white transition-colors">← Exit Pipeline</a>
			<span class="w-1.5 h-1.5 rounded-full bg-white/10"></span>
			<a href="/dashboard" class="text-[11px] font-bold text-white/30 uppercase tracking-widest hover:text-white transition-colors">User Portal</a>
		</div>
	</div>
</div>
