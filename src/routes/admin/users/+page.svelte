<script lang="ts">
	...
	let loadingUsers = $derived(usersQuery.isLoading);
...
{#snippet roleSnippet(role: string)}
	<div class="flex items-center gap-2">
		<div class="w-2 h-2 rounded-full" style="background: {role === 'admin' ? '#ef4444' : '#6b7280'};"></div>
...
<div class="space-y-8">
	...
	<div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
		<div>
			<h1 class="font-display text-3xl text-white mb-2">User Management</h1>
			<p class="text-white/40 text-sm">Live directory of all registered accounts across the platform.</p>
		</div>
...
>
	<div class="space-y-8">
		...
		<!-- Permission Toggle -->
		<div class="space-y-4">
			<h5 class="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] px-1">Access Control</h5>
			<div class="flex items-center justify-between p-4 rounded-2xl bg-black/40 border border-white/5">
				<div>
					...
					<div class="text-[10px] text-white/20 uppercase tracking-widest mt-1">Full Platform Access</div>
				</div>
				<button 
					onclick={toggleRole}
		...
		<!-- Identity Details -->
		<div class="space-y-4">
			<h5 class="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] px-1">Profile Details</h5>
			<div class="grid grid-cols-2 gap-3">
				<div class="p-4 rounded-2xl bg-white/5 border border-white/5">
					<div class="text-[9px] text-white/20 uppercase tracking-widest mb-1">State</div>
					<div class="text-xs font-bold text-white">{selectedUser?.stateOfOrigin || 'Not set'}</div>
				</div>
				<div class="p-4 rounded-2xl bg-white/5 border border-white/5">
					<div class="text-[9px] text-white/20 uppercase tracking-widest mb-1">Plan</div>
					<div class="text-xs font-bold text-lime-500">{selectedUser?.plan || 'free'}</div>
				</div>
				<div class="p-4 rounded-2xl bg-white/5 border border-white/5">
					<div class="text-[9px] text-white/20 uppercase tracking-widest mb-1">Institution</div>
					<div class="text-xs font-bold text-white truncate">{selectedUser?.institutionName || 'Not set'}</div>
				</div>
				<div class="p-4 rounded-2xl bg-white/5 border border-white/5">
					<div class="text-[9px] text-white/20 uppercase tracking-widest mb-1">Level</div>
					<div class="text-xs font-bold text-white">{selectedUser?.level || 'Not set'}</div>
				</div>
			</div>
		</div>
		...
		<div class="space-y-4 pt-6 border-t border-white/5">
			<h5 class="text-[10px] font-bold text-rose-500/50 uppercase tracking-[0.2em] px-1">Danger Zone</h5>
	...
	{#snippet footer()}
		<div class="flex gap-4">
			<button class="btn-ghost flex-1 h-[52px] rounded-xl text-xs font-bold tracking-tight" onclick={() => isDrawerOpen = false}>CLOSE</button>
			<button class="btn-violet flex-1 h-[52px] rounded-xl text-xs font-bold tracking-tight shadow-xl">SEND MESSAGE</button>
		</div>
	{/snippet}
</Drawer>
