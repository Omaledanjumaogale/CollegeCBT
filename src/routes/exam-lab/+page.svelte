<script lang="ts">
	...
	function answerMCQ(key: string) {
		...
		labAnswered = true;
		selectedOption = key;
		const isCorrect = key === labQuestion.answer;
		if (isCorrect) {
			labStats.correct++;
			labStats.score += 2;
		...
		// Save practice telemetry if authenticated
		if ($currentUser?.uid) {
	...
	async function finishMock() {
		...
		// Save mock exam telemetry if authenticated
		if ($currentUser?.uid) {
			try {
				const { saveStudySession } = await import('$lib/services/convexClient');
				await saveStudySession($currentUser.uid, {
					...
					correct,
					wrong: mockQuestions.length - correct,
					score: pct,
					mode: 'mock',
					grade,
					timestamp: Date.now()
				});
				showToast('✅ Progress Saved', 'Your exam result has been synchronized with your profile.', 'success');
			} catch (err) {
				console.error('[CollegeCBT] Failed to save session:', err);
			}
		}
	}
